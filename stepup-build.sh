...
UID=${UID} GID=${GID} docker-compose -f ../docker-compose.yml exec -T build-container bash -c "./prepare-container.sh Stepup-SelfService"
# "tmp/build.XXXXXXXX" is 18 characters long
UID=${UID} GID=${GID} docker-compose -f ../docker-compose.yml exec -T build-container bash -c "./stepup-build2.sh ${TMP_ARCHIVE_DIR:(-18)} ${COMPONENT} ${NAME}"
if [ $# -ne "0" ]; then
